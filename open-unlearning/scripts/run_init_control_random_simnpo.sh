...
MANIFEST="logs/init_control_random_simnpo_${FORGET_SPLIT}_${TS}.tsv"
printf "timestamp\tseed\tsteps\tstatus\n" > "${MANIFEST}"

for SEED in ${SEEDS}; do
  OUT="saves/unlearn/INITCTRL_TOFU_${FORGET_SPLIT}_random_rank2_matched_kforge_s045_seed${SEED}"
  if [[ ! -s "${OUT}/config.json" ]]; then
    CUDA_VISIBLE_DEVICES="${GPU_ID}" python scripts/make_matched_init_checkpoint.py \
      --base-model "${BASE_MODEL}" \
      --template-model "${TEMPLATE}" \
      --output-dir "${OUT}" \
      --mode random_low_rank \
      --rank 2 \
      --seed "${SEED}" \
      --device cuda \
      --dtype float32 \
      2>&1 | tee "logs/initctrl_make_random_seed${SEED}_${TS}.log"
  fi

  for STEPS in ${STEP_BUDGETS}; do
    TASK="tofu_${MODEL_ID}_${FORGET_SPLIT}_SimNPO_random_rank2_S${STEPS}_seed${SEED}_${RUN_TAG}"
