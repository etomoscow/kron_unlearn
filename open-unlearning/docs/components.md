...
K-FORGE is available as a one-shot unlearning trainer through
`configs/trainer/KFORGE.yaml`. It estimates forget and retain Kronecker-Fisher
factors for selected linear layers, computes a damped rank-r generalized SVD
edit, and applies it without optimizer steps. See `docs/kforge.md` for the
method details and tuning knobs.
